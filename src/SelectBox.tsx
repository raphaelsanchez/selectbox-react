...
const SelectBox: React.FC<SelectBoxProps> = React.memo(
  ({
      ...
      (event: KeyboardEvent<HTMLDivElement>) => {
        ...
        switch (event.key) {
          ...
          case " ":
            event.preventDefault();
            if (focusedIndex !== -1) {
              handleSelectOption(filteredOptions[focusedIndex].name);
            }
            break;
          case "Escape":
            event.preventDefault();
    ...
    useEffect(() => {
      if (isOpen && focusedIndex !== -1 && optionRefs.current[focusedIndex]) {
    ...
    return (
      <div ref={selectBoxRef} className="relative" onKeyDown={handleKeyDown}>
        {label && (
        ...
        <button
          ...
          aria-labelledby={
            label ? `${name}-label ${name}-button` : `${name}-button`
          }
          ...
          >
            ...
            <ul
              ...
              aria-activedescendant={
                focusedIndex !== -1 && filteredOptions[focusedIndex]
